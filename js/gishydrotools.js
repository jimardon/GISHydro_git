...
function basin_properties(){
    ...
    function basinpropsCallback(error, response, raw){
        ...
        for(var j=0; j < regioncount; j++){
            ...
            basin3_modal +=             '<button type="button" class="btn btn-default" onclick=modaltotxt(tasker_mod,"' + full_project_name + '_' + provstring[j][0].replace(' ','') + '_frre.csv")>Download</button>'
...
function createtctable(subarea,Pixel,Type,Elev,Slope,AvgArea,Width,Depth,Xarea,Tot_Length,Vel,I_Time,Tot_Time){
    ...
    tc_modal +=         '</div>';
    tc_modal +=         '<div class="modal-footer" style="justify-content: space-between;">';
    tc_modal +=             '<button type="button" class="btn btn-default" onclick=modaltotxt("tc_modal' + String(subarea+1) + '",' + full_project_name + '_velmeth' + String(subarea+1) + '.csv")>Download</button>'
