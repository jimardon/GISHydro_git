...
function recalculatetc(){
    ...
    if (single_channel == 1 && length(channel_list) != 0){
        ...
        avgarea = 0
        for(var i=0; i < pixel_channel.length; i++){
            avgarea = (dspx_channel[i] - uspx_channel[i]) * avgarea_channel[i] + avgarea
        }
        avgarea = avgarea / (uspx_channel[uspx_channel.length-1] - dspx_channel[0])
        upelev = elev_channel[0]
        dwnelev = elev_channel[elev_channel.length-1]
        ilength = ilength_channel.reduce(function(a, b){return a + b;}, 0);
        slope = (upelev - dwnelev) / ilength
    ...
    if (len(overland_list) != 0 && length(swale_list) != 0){
        ...
        for(var i=0; i < swale_list.length; i++){
